...
export default function TutorialTooltip({ rect, message, placement = 'top', onConfirm, warning, warn, center, rightAlign, minWidth, multiLine, textAlign }) {
  if (!rect) return null

  const left = rightAlign ? rect.right : (center ? rect.left + rect.width / 2 : Math.max(rect.left, 12))
  const shift = rightAlign ? 'translateX(-100%)' : (center ? 'translateX(-50%)' : '')
  ...
  const tailLeft = rightAlign ? undefined : (center ? '50%' : 16)
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'absolute', left: tailLeft, right: tailRight, width: 14, height: 14, background: '#fff',
