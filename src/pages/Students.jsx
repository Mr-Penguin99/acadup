...
const STUDENT_STATUS_DATA = [
  ...
  { id:2, name:'@이순신',  birth:'20.01.01', photo:'X', status:'재원', classes:['to_반그룹 > to_반_AAA_배정','to_반그룹 > from_반_CCC','to_반그룹 > to_반_001_배정','고등_AA > 고등_AA_기초반','test02','to_반그룹 > from_반_CCC'], keypad:'2350', dept:'초등학교', school:'', grade:'1' },
  ...
  { id:7, name:'김학생AA', birth:'23.01.01', photo:'X', status:'재원', classes:['to_반그룹 > to_반_001_배정','to_반그룹 > to_반_111_배정','반그룹_01(기간반) > 01_국어(222개월)'], keypad:'', dept:'', school:'', grade:'' },
...
const [attendFilter, setAttendFilter] = useState({
  month: '2026-05', searchType: '반그룹', group: '전체',
  className: '', studentStatus: '전체', name: ''
})
...
const toggleReplaceCheck = id =>
  setReplaceChecked(c => c.includes(id) ? c.filter(x => x !== id) : [...c, id])
const [talkChecked, setTalkChecked] = useState([])
const toggleTalkCheck = id =>
  setTalkChecked(c => c.includes(id) ? c.filter(x => x !== id) : [...c, id])
const [noticeChecked, setNoticeChecked] = useState([])
const toggleNoticeCheck = id =>
  setNoticeChecked(c => c.includes(id) ? c.filter(x => x !== id) : [...c, id])

const toggleStatusCheck = id =>
  setStatusChecked(c => c.includes(id) ? c.filter(x => x !== id) : [...c, id])
const toggleStatusAll = () =>
  setStatusChecked(statusChecked.length === STUDENT_STATUS_DATA.length ? [] : STUDENT_STATUS_DATA.map(d => d.id))
  const [form, setForm] = useState({
    studentNo: '', status: '',
    name: '', birth: '', gender: '남자',
    id: '', pw: '',
    enrollDate: '', payMethod: '',
    phone: '', homePhone: '',
    grade1: '', grade2: '', attendNo: '',
  ...
  {activeSide === 'class-students' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#F5C518'}}>⭐</span> 반별 수강생 (직원용)
      </div>
      <div className="students-content">
        <div className="search-panel">
          <div className="sp-tabs">
            {['반','강사'].map(t => (
              <button key={t}
                className={`sp-tab ${searchTab === t ? 'active' : ''}`}
                onClick={() => setSearchTab(t)}>{t}</button>
            ))}
          </div>
          <div className="sp-form">
            <div className="sp-field">
              <label className="sp-label">반그룹</label>
              <select className="sp-input"><option>선택하기</option></select>
            </div>
            <div className="sp-field">
              <label className="sp-label">반명</label>
              <select className="sp-input"><option>선택하기</option></select>
            </div>
            <div className="sp-field">
              <label className="sp-label">수강생</label>
              <input className="sp-input" placeholder="성명 또는 휴대폰 끝4자리"
                value={search.name}
              ...
              <div className="info-form">
                <div className="if-row">
                  <div className="if-field">
                    ...
                    <div style={{display:'flex', gap:6, alignItems:'center'}}>
                      <input className="if-input" placeholder="미입력시 자동부여" value={form.studentNo}
                  ...
                  <div className="if-field">
                    ...
                    <select className="if-input" style={{width:120}}>
                ...
                <div className="if-row">
                  <div className="if-field">
                    ...
                    <input className="if-input" value={form.name}
                      onChange={e => setForm(f => ({...f, name: e.target.value}))} />
                  </div>
                  <div className="if-field">
                    <label className="if-label required">생년월일</label>
                    <div style={{display:'flex', gap:6, alignItems:'center'}}>
                      <input className="if-input" placeholder="예) 901230" style={{width:120}} value={form.birth}
                        onChange={e => setForm(f => ({...f, birth: e.target.value}))} />
                      <select className="if-input" style={{width:80}} value={form.gender}
                ...
                <div className="if-row">
                  <div className="if-field">
                    ...
                    <div style={{display:'flex', gap:6, alignItems:'center'}}>
                      <input className="if-input" value={form.id}
                ...
                <div className="if-row">
                  <div className="if-field">
                    ...
                    <input className="if-input" type="date" value={form.enrollDate}
                      onChange={e => setForm(f => ({...f, enrollDate: e.target.value}))} />
                  </div>
                  <div className="if-field">
                    <label className="if-label">주 결제방법</label>
                    <select className="if-input">
                      <option>선택</option><option>카드</option><option>현금</option><option>계좌이체</option>
                    </select>
                  </div>
                </div>
                <div className="if-row">
                  <div className="if-field">
                    ...
                    <input className="if-input" placeholder="예) 010-1234-5678" value={form.phone}
                      onChange={e => setForm(f => ({...f, phone: e.target.value}))} />
                  </div>
                  <div className="if-field">
                    <label className="if-label">집전화</label>
                    <input className="if-input" placeholder="예) 010-1234-5678" value={form.homePhone}
                ...
                <div className="if-row">
                  <div className="if-field">
                    ...
                    <div style={{display:'flex', gap:6, alignItems:'center'}}>
                      ...
                      <input className="if-input" style={{width:80}} value={form.grade2}
                        onChange={e => setForm(f => ({...f, grade2: e.target.value}))} />
                      <span className="if-hint">학년 0</span>
                    </div>
                  </div>
                  <div className="if-field">
                    <label className="if-label">출결번호</label>
                    <input className="if-input" placeholder="숫자 4자리 입력" maxLength={4}
                      value={form.attendNo}
                      onChange={e => setForm(f => ({...f, attendNo: e.target.value}))} />
                  </div>
                </div>
                <div className="if-row">
                  <div className="if-field">
                    ...
                    <div style={{display:'flex', gap:6, alignItems:'center'}}>
                      <input className="if-input" style={{flex:1}} value={form.email1}
                        onChange={e => setForm(f => ({...f, email1: e.target.value}))} />
                      <span>@</span>
                      <input className="if-input" style={{flex:1}} value={form.email2}
                        onChange={e => setForm(f => ({...f, email2: e.target.value}))} />
                      <select className="if-input" style={{width:90}}>
                        <option>직접입력</option><option>gmail.com</option><option>naver.com</option>
                      </select>
                    </div>
                  </div>
                </div>
              ...
              <div className="info-tabs">
                ...
                {INFO_TABS.map(t => (
                  <button key={t}
                    ...
                    onClick={() => setInfoTab(t)}>{t}</button>
                ))}
              </div>
              <div className="info-tab-content">
                <p className="empty-msg">{infoTab} 정보가 없습니다.</p>
              </div>
            </div>
          </div>
        </div>
  ...
  {activeSide === 'student-status' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#F5C518'}}>⭐</span> 수강생 현황
      </div>
        ...
        <div className="sts-filter">
          <div className="sts-filter-row">
            <div className="sts-filter-item">
              <label className="sts-filter-label">강사</label>
              <select className="sts-input" value={statusFilter.teacher}
                onChange={e => setStatusFilter(f => ({...f, teacher: e.target.value}))}>
                <option>전체</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">반 그룹</label>
              <select className="sts-input" value={statusFilter.group}
                onChange={e => setStatusFilter(f => ({...f, group: e.target.value}))}>
                <option>전체</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">반명</label>
              <select className="sts-input" value={statusFilter.className}
                onChange={e => setStatusFilter(f => ({...f, className: e.target.value}))}>
                <option>전체</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">수강생상태</label>
              <select className="sts-input" value={statusFilter.studentStatus}
                onChange={e => setStatusFilter(f => ({...f, studentStatus: e.target.value}))}>
                <option>재원</option><option>휴원</option><option>퇴원</option><option>예비</option>
              </select>
            </div>
            <div className="sts-filter-item" style={{flex:2}}>
              <label className="sts-filter-label">검색</label>
              <div style={{display:'flex', gap:6}}>
                <select className="sts-input" style={{width:120}} value={statusFilter.searchType}
                  onChange={e => setStatusFilter(f => ({...f, searchType: e.target.value}))}>
                  <option>수강생-성명</option><option>수강생-전화</option><option>보호자-성명</option>
                </select>
                <input className="sts-input" style={{flex:1}} value={statusFilter.keyword}
                  onChange={e => setStatusFilter(f => ({...f, keyword: e.target.value}))} />
              </div>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">수강동기</label>
              <select className="sts-input" value={statusFilter.motive}
                onChange={e => setStatusFilter(f => ({...f, motive: e.target.value}))}>
                <option>선택하기</option>
              </select>
            </div>
          </div>
          <div className="sts-filter-row" style={{marginTop:8}}>
            <div className="sts-filter-item">
              <label className="sts-filter-label">학부</label>
              <select className="sts-input" value={statusFilter.dept}
                onChange={e => setStatusFilter(f => ({...f, dept: e.target.value}))}>
                <option>선택하기</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">학년</label>
              <input className="sts-input" value={statusFilter.grade}
                onChange={e => setStatusFilter(f => ({...f, grade: e.target.value}))} />
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">학교</label>
              <input className="sts-input" value={statusFilter.school}
                onChange={e => setStatusFilter(f => ({...f, school: e.target.value}))} />
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">성별</label>
              <select className="sts-input" value={statusFilter.gender}
                onChange={e => setStatusFilter(f => ({...f, gender: e.target.value}))}>
                <option>선택하기</option><option>남자</option><option>여자</option>
              </select>
            </div>
            <div className="sts-filter-item" style={{flex:2}}>
              <label className="sts-filter-label">생년월일</label>
              <div style={{display:'flex', gap:6, alignItems:'center'}}>
                <input type="date" className="sts-input" value={statusFilter.birthFrom}
                  onChange={e => setStatusFilter(f => ({...f, birthFrom: e.target.value}))} />
                <span>~</span>
                <input type="date" className="sts-input" value={statusFilter.birthTo}
                  onChange={e => setStatusFilter(f => ({...f, birthTo: e.target.value}))} />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="sts-section">
        <div className="sts-list-head">
          <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
            ...
            <span className="sts-count-info">
              검색인원:12명 &nbsp;
              <span style={{color:'#555'}}>(전체수강생)</span>
              &nbsp;재원:<strong style={{color:'#333'}}>11명</strong>
              &nbsp;예비:<strong style={{color:'#29ABE2'}}>3명</strong>
              &nbsp;휴원:<strong>0명</strong>
              &nbsp;퇴원:<strong style={{color:'#E8445A'}}>33명</strong>
            </span>
          </div>
          <div style={{display:'flex', alignItems:'center', gap:6}}>
            <span style={{fontSize:12, color:'#666'}}>페이지당 조회</span>
            <select className="sts-input" style={{width:60}} value={statusPageSize}
              onChange={e => setStatusPageSize(e.target.value)}>
            ...
            <thead>
              <tr>
                <th><input type="checkbox"
                  checked={statusChecked.length === STUDENT_STATUS_DATA.length}
                  onChange={toggleStatusAll} /></th>
                <th>성명</th>
                <th>생년월일</th>
                <th>사진</th>
                <th>상태</th>
              ...
              {STUDENT_STATUS_DATA.map(d => (
                <tr key={d.id} className={statusChecked.includes(d.id) ? 'checked-row' : ''}>
                  <td><input type="checkbox"
                    checked={statusChecked.includes(d.id)}
                    onChange={() => toggleStatusCheck(d.id)} /></td>
                  <td>
        ...
        <div className="sts-pagination">
          <div className="sts-pages">
            {[1, 2].map(p => (
              <button key={p} className={`sts-page-btn ${p === 1 ? 'active' : ''}`}>{p}</button>
            ))}
          </div>
  ...
  {activeSide === 'attend-inout' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#ccc'}}>☆</span> 등원/하원
      </div>
        ...
        <div className="sts-filter">
          <div className="sts-filter-row">
            <div className="sts-filter-item">
              ...
              <input type="month" className="sts-input" value={attendFilter.month}
                onChange={e => setAttendFilter(f => ({...f, month: e.target.value}))} />
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">검색유형</label>
              <select className="sts-input" value={attendFilter.searchType}
                onChange={e => setAttendFilter(f => ({...f, searchType: e.target.value}))}>
                <option>반그룹</option><option>반명</option><option>수강생명</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">반 그룹</label>
              <select className="sts-input" value={attendFilter.group}
                onChange={e => setAttendFilter(f => ({...f, group: e.target.value}))}>
                <option>전체</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">반명</label>
              <select className="sts-input" value={attendFilter.className}
                onChange={e => setAttendFilter(f => ({...f, className: e.target.value}))}>
                <option>선택하기</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">수강생상태</label>
              <select className="sts-input" value={attendFilter.studentStatus}
                onChange={e => setAttendFilter(f => ({...f, studentStatus: e.target.value}))}>
                <option>전체</option><option>재원</option><option>휴원</option><option>퇴원</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">수강생 성명</label>
              <input className="sts-input" value={attendFilter.name}
                onChange={e => setAttendFilter(f => ({...f, name: e.target.value}))} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="sts-section">
        <div className="att-cal-head">
          <span className="sts-sec-title">등원/하원 현황</span>
        </div>
              ...
              <tr>
                ...
                {(() => {
                  ...
                  return Array.from({length: days}, (_, i) => {
                    ...
                    return (
                      <th key={d} className={`att-th-day ${dow===0?'att-sun':dow===6?'att-sat':''}`}>
                        <div>{d}일</div>
                        <div>{DOW[dow]}</div>
                      </th>
                    )
              ...
              {ATTEND_DATA.map(d => (
                <tr key={d.id}>
                  ...
                  {(() => {
                    ...
                    const days = new Date(y, m, 0).getDate()
                    return Array.from({length: days}, (_, i) => (
                      <td key={i} className="att-td-day"></td>
                    ))
                  })()}
                </tr>
              ))}
  ...
  {activeSide === 'attend-ride' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#ccc'}}>☆</span> 승차/하차
      </div>
      ...
      <div className="sts-section">
        ...
        <div className="sts-filter">
          <div className="sts-filter-row">
            <div className="sts-filter-item">
              <label className="sts-filter-label">수강월</label>
              <input type="month" className="sts-input" value={rideFilter.month}
                onChange={e => setRideFilter(f => ({...f, month: e.target.value}))} />
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">반 그룹</label>
              <select className="sts-input" value={rideFilter.group}
                onChange={e => setRideFilter(f => ({...f, group: e.target.value}))}>
                <option>전체</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">반명</label>
              <select className="sts-input" value={rideFilter.className}
                onChange={e => setRideFilter(f => ({...f, className: e.target.value}))}>
                <option>선택하기</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">수강생상태</label>
              <select className="sts-input" value={rideFilter.studentStatus}
                onChange={e => setRideFilter(f => ({...f, studentStatus: e.target.value}))}>
                <option>전체</option><option>재원</option><option>휴원</option><option>퇴원</option>
              </select>
            </div>
            <div className="sts-filter-item">
              <label className="sts-filter-label">수강생 성명</label>
              <input className="sts-input" value={rideFilter.name}
                onChange={e => setRideFilter(f => ({...f, name: e.target.value}))} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="sts-section">
        <div className="att-cal-head">
          <span className="sts-sec-title">승차/하차 현황</span>
        </div>
        <div className="att-cal-wrap">
          <table className="att-cal-table">
            <thead>
              <tr>
                ...
                <th className="att-th-name">성명</th>
                {(() => {
                  const [y, m] = rideFilter.month.split('-').map(Number)
                  const days = new Date(y, m, 0).getDate()
                  const DOW = ['일','월','화','수','목','금','토']
                  return Array.from({length: days}, (_, i) => {
                    const d = i + 1
  ...
  {activeSide === 'notice-board' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#ccc'}}>☆</span> 공지사항
      </div>

      <div className="notice-subtitle">
        수강생 및 학부모에게 안내하는 경리up다(학원)ㄱㄱ 소식입니다.
      </div>

      <div className="notice-search-wrap">
        <select className="sts-input" style={{width:120}} value={noticeSearch.type}
          onChange={e => setNoticeSearch(f => ({...f, type: e.target.value}))}>
          <option>제목+내용</option>
          <option>제목</option>
          <option>내용</option>
        </select>
        <div className="notice-search-box">
          <input className="notice-search-input" value={noticeSearch.keyword}
            onChange={e => setNoticeSearch(f => ({...f, keyword: e.target.value}))}
            placeholder="" />
          <button className="notice-search-icon">🔍</button>
        </div>
      </div>
      ...
      <div className="sts-section">
        <div className="notice-list-head">
          ...
          <div style={{display:'flex', gap:6}}>
            <button className="sts-teal-btn">공지사항 등록</button>
            ...
            {NOTICE_DATA.map(d => (
              <tr key={d.id}>
                ...
                <td style={{textAlign:'center'}}>
                  <input type="checkbox"
                    checked={noticeChecked.includes(d.id)}
                    onChange={() => toggleNoticeCheck(d.id)} />
                </td>
                <td>
                  <div style={{display:'flex', alignItems:'center', gap:6}}>
                    {d.type && <span className="notice-badge">{d.type}</span>}
                    <span className="notice-file-icon">📄</span>
                    {d.linked
                      ? <span className="notice-title-link">{d.title}</span>
                      : <span>{d.title}</span>
                    }
                  </div>
                </td>
                <td style={{textAlign:'center'}}>
                  <span className={d.sentStatus === '미전송' ? 'notice-unsent' : 'notice-sent'}>
                    {d.sentStatus}
                  </span>
                </td>
  ...
  {activeSide === 'notice-talk' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#ccc'}}>☆</span> 소통톡톡
      </div>

      <div className="notice-subtitle">
        수강생의 멋진 작품과 성장 이야기, 학원 생활에서의 소중한 순간들을 전합니다.
      </div>

      <div className="notice-search-wrap">
        <select className="sts-input" style={{width:120}} value={talkSearch.type}
          onChange={e => setTalkSearch(f => ({...f, type: e.target.value}))}>
          <option>제목+내용</option>
          <option>제목</option>
          <option>내용</option>
        </select>
        <div className="notice-search-box">
          <input className="notice-search-input" value={talkSearch.keyword}
            onChange={e => setTalkSearch(f => ({...f, keyword: e.target.value}))} />
          <button className="notice-search-icon">🔍</button>
        </div>
      </div>
      ...
      <div className="sts-section">
        <div className="notice-list-head">
          ...
          <div style={{display:'flex', gap:6}}>
            <button className="sts-teal-btn">소통톡톡 등록</button>
            <button className="sts-export-btn">알림톡전송</button>
          </div>
        </div>
        <table className="notice-table">
          ...
          <tbody>
            {TALK_DATA.map(d => (
              <tr key={d.id}>
                ...
                <td style={{textAlign:'center'}}>
                  <input type="checkbox"
                    checked={talkChecked.includes(d.id)}
                    onChange={() => toggleTalkCheck(d.id)} />
                </td>
                <td>{d.title}</td>
                <td style={{textAlign:'center'}}>
                  <span className={d.sentStatus === '미전송' ? 'notice-unsent' : 'notice-sent'}>
  ...
  {activeSide === 'notice-replace' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#ccc'}}>☆</span> 문자치환 공지
      </div>
      ...
      <div className="notice-search-wrap">
        <select className="sts-input" style={{width:120}} value={replaceSearch.type}
          onChange={e => setReplaceSearch(f => ({...f, type: e.target.value}))}>
          <option>제목+내용</option>
          <option>제목</option>
          <option>내용</option>
        </select>
        <div className="notice-search-box">
          <input className="notice-search-input" value={replaceSearch.keyword}
            onChange={e => setReplaceSearch(f => ({...f, keyword: e.target.value}))} />
          <button className="notice-search-icon">🔍</button>
        </div>
      </div>
      ...
      <div className="sts-section">
        <div className="notice-list-head">
          <span className="sts-sec-title">문자치환 공지목록</span>
          <button className="sts-export-btn">신규 등록</button>
        </div>
        <table className="notice-table">
          <thead>
            <tr>
          ...
          <tbody>
            {REPLACE_DATA.map(d => (
              <tr key={d.id}>
                ...
                <td style={{textAlign:'center'}}>
                  <input type="checkbox"
                    checked={replaceChecked.includes(d.id)}
                    onChange={() => toggleReplaceCheck(d.id)} />
                </td>
                <td>
                  <div style={{display:'flex', alignItems:'center', gap:6}}>
                    {d.hasFile && <span className="notice-file-icon">📄</span>}
                    <span>{d.title}</span>
                  </div>
                </td>
  ...
  {activeSide === 'notice-schedule' && (
    <>
      <div className="sm-page-title">
        <span style={{color:'#ccc'}}>☆</span> 전송예약 현황
      </div>
        ...
        <div className="sts-filter">
          <div className="sts-filter-row">
            <div className="sts-filter-item" style={{flex:'none'}}>
              ...
              <div style={{display:'flex', gap:6}}>
                <input className="sts-input" style={{width:200}} value={scheduleSearch}
      ...
      <div className="sts-section">
        <div className="notice-list-head">
          <span className="sts-sec-title">예약 목록</span>
          <button className="sts-search-btn" style={{background:'#E8445A'}}>전송 취소</button>
        </div>
        <table className="notice-table">
          <thead>
            <tr>
              <th><input type="checkbox" onChange={e =>
                setScheduleChecked(e.target.checked ? ['all'] : [])} /></th>
              <th>수신번호</th>
              <th>예약시간</th>
              <th>메세지</th>
              <th>등록자</th>
          ...
          <tbody>
            <tr>
              <td colSpan={6} style={{textAlign:'center', padding:'40px', color:'#bbb', fontSize:13}}>
                검색된 데이터가 없습니다.
              </td>
            </tr>
          </tbody>
